...
### 5. 회전 방해토크(τ_disturbance) 확장 — 발산 해결, 순이익 문턱 직전까지
...
- **해결책**: 데이터 수집 중에 실제로 무작위 토크를 주입(더더링, 개루프라
  ...
  순이익까지는 한 걸음 남았지만 궤적은 명확히 좋은 방향. 상세 기록은
  EXPERIMENTS.md 12-16/12-17절 참고. 병진 쪽 성과(위 4절)는 이 실험과 무관하게
  그대로 유효함.
...
| PX4 토크 피드포워드 A/B (더더링 데이터 재학습, 3조건) | 자세 피크 편차 개선율 | calm -97.5%, default -2.7%, strong -5.8% (발산 없음, 사실상 중립) |
...
- **(완료) 회전 방해토크 확장** — 오프라인 학습·5-fold 검증 성공, PX4 펌웨어 토크
  피드포워드 채널 구현·캘리브레이션 완료. 게인 스윕/저역통과 필터는 둘 다 실패로
  결론 남. **근본 원인(무보정 데이터로 인한 자기입력 미분 지름길) 재진단 후
  토크 더더링으로 데이터 재수집(15세션) + 재학습 + 재검증 — 발산 완전히 해소,
  default/strong 조건 사실상 중립(-2.7%/-5.8%)까지 도달** (EXPERIMENTS.md
  12-16/12-17절)
- **회전 피드포워드 순이익 마지막 한 걸음** — `TAU_FF_GAIN`을 더 낮춰보기(현재
  1.0), yaw 해상도/조건 수를 더 늘려 재수집, 더더링 강도 자체 조정 등 후보
  (12-17절 "다음 단계")
