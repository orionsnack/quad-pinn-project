...
- 실시간 A/B 1차 시도에서 **폐루프 발산**(무풍에서도 21°+) — 원인은 모델 입력
  ...
  전부 여전히 순이익 없음**(추정 정확도가 함께 떨어진 게 원인으로 추정).
  이후 시도한 두 후보도 실패: `TAU_FF_GAIN` 스윕은 게인에 거의 선형 비례해서
  손해가 커지는 패턴만 확인(튜닝으로 해결 불가 확정), 저역통과 필터로 각속도를
  다시 넣은 버전은 오프라인 정확도는 회복했지만 **폐루프가 오히려 더 심하게
  발산**(38.7° — 위상지연이 안정성을 더 해침). 상세 기록은 EXPERIMENTS.md
  12-16절 참고. 병진 쪽 성과(위 4절)는 이 실험과 무관하게 그대로 유효함.
...
- **(완료) 회전 방해토크 확장** — 오프라인 학습·5-fold 검증 성공, PX4 펌웨어 토크
  ...
  이후 시도한 `TAU_FF_GAIN` 스윕(튜닝으로 해결 불가 확정)과 저역통과 필터 재도입
  (오히려 더 심하게 발산)도 둘 다 실패로 결론 남 (EXPERIMENTS.md 12-16절)
- **회전 피드포워드 실시간 순이익 확보** — 입력 신호 가공(무필터/제외/LPF) 3가지가
  전부 막혔으므로, 남은 유일한 후보는 회전 전용 짧은 윈도우 서브모델(모델 구조
  자체를 바꾸는 방향) (12-16절 "다음 단계")
