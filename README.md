...
- **`TAU_FF_GAIN` 재스윕으로 첫 순이익 달성**: 더더링 이전엔 게인 튜닝이 막힌
  ...
  다시 스윕해보니 gain=1.0→0.2로 낮추면서 부호가 바뀜 — **default +2.8%~+3.8%
  (재현 확인), strong +3.2% 개선**, calm은 절대오차 0.036°로 사실상 중립.
  세미나 원 과제(회전 방해토크 실시간 피드포워드)가 실제로 작동하고 도움이
  되는 첫 사례. 상세 기록은 EXPERIMENTS.md 12-16/12-17절 참고. 병진 쪽 성과
  (위 4절)는 이 실험과 무관하게 그대로 유효함.
...
| PX4 토크 피드포워드 A/B (더더링 데이터 재학습, gain=0.2) | 자세 피크 편차 개선율 | default +2.8~3.8%, strong +3.2% 개선, calm 사실상 중립 |
...
- **(완료) 회전 방해토크 확장, 첫 순이익 달성** — 오프라인 학습·5-fold 검증 성공,
  PX4 펌웨어 토크 피드포워드 채널 구현·캘리브레이션 완료. 게인 스윕/저역통과
  필터는 둘 다 실패로 결론 남. **근본 원인(무보정 데이터로 인한 자기입력 미분
  지름길) 재진단 후 토크 더더링으로 데이터 재수집(15세션) + 재학습 + 재검증 —
  발산 완전히 해소. 이 새 모델로 게인 재스윕(gain=0.2)해서 default +2.8~3.8%,
  strong +3.2% 실제 개선 달성**(초기 시도 대비 손해에서 순이익으로 완전히 반전)
  (EXPERIMENTS.md 12-16/12-17절)
- **회전 피드포워드 마무리 검증** (우선순위 낮음) — gain=0.2가 다른 yaw/조건
  전반에서도 안정적인지 더 폭넓게 검증, `TAU_FF_GAIN`을 물리 단위로 정밀
  캘리브레이션 (12-17절 "다음 단계")
