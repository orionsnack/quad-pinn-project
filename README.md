...
- **5조건(병진 A/B와 동일한 calm/light/default/strong/crosswind)으로 확장
  ...
  패턴**으로 재현성 확인. 세미나 원 과제(회전 방해토크 실시간 피드포워드)가
  실제로 작동하고 도움이 되는 첫 사례. 상세 기록은 EXPERIMENTS.md 12-16/12-17절
  참고. 병진 쪽 성과(위 4절)는 이 실험과 무관하게 그대로 유효함.
...
- **(완료) 회전 방해토크 확장, 첫 순이익 달성 + 5조건 재현 검증** — 오프라인
  학습·5-fold 검증 성공, PX4 펌웨어 토크 피드포워드 채널 구현·캘리브레이션
  완료. 게인 스윕/저역통과 필터는 둘 다 실패로 결론 남. **근본 원인(무보정
  데이터로 인한 자기입력 미분 지름길) 재진단 후 토크 더더링으로 데이터
  재수집(15세션) + 재학습 + 재검증 — 발산 완전히 해소. 이 새 모델로 게인
  재스윕(gain=0.2)해서 순이익 달성, 병진 A/B와 동일한 표준 5조건으로 재검증한
  결과 5개 중 3개 개선(+1.0~5.5%)으로 병진과 거의 같은 패턴 확인**(초기 시도
  대비 손해에서 순이익으로 완전히 반전) (EXPERIMENTS.md 12-16/12-17절)
- **회전 피드포워드 마무리 검증** (우선순위 낮음) — 다른 yaw 각도에서도 이
  패턴이 유지되는지 검증(지금까지 전부 고정 yaw에서만 테스트), `TAU_FF_GAIN`을
  물리 단위로 정밀 캘리브레이션 (12-17절 "다음 단계")
