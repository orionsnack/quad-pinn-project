...
- **회전 방해토크(τ_disturbance) 실시간 폐루프 개선** (12-16절 참고) — 각속도 입력을
  ...
  - 각속도를 입력에 다시 넣되 저역통과 필터(느린 시상수)로 지연시켜서 즉각적인
    되먹임 경로만 끊는 절충안 — 재학습 없이 바로 시도 가능, 다만 루프 안정성
    보장은 안 됨(미검증) — **다음 시도 후보 1순위**
  - 별도의 짧은 윈도우(예: 최근 4~5스텝)로 회전 전용 서브모델을 분리 학습 —
    병진(1초 윈도우)과 회전(수백ms 윈도우)의 시간 스케일이 다르다는 12-16절
    분석과 맞물림, 다만 모델 구조 변경이 필요해 비용 높음
