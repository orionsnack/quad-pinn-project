...
| 위 `parameters.bson` 삭제/`/tmp/px4_lock-0`·`/tmp/px4-sock-0` 정리로도 GPS/홈 위치 확인이 영영 안 넘어감(raw GPS는 `gps_info`/`raw_gps`로 보면 위성 10개·FIX_3D·hdop<1로 정상인데 `is_global_position_ok`만 계속 False, 180초 넘게 기다려도 그대로) | `build/px4_sitl_default` 디렉터리 자체가 (parameters.bson보다 더 깊은 수준으로) 오염된 것으로 추정 — `kill -9`를 아주 여러 번(수십 회) 반복하면 언젠가 이 상태에 빠지는 것으로 보이며, 2026-08-13~14에 두 번 재현·재해결됨. **해결책은 완전 클린 재빌드뿐**: `rm -rf ~/MyProjects/PX4-Autopilot/build/px4_sitl_default` 후 `HEADLESS=1 make px4_sitl gz_x500_windy`(수 분 소요)로 처음부터 다시 빌드하면 GPS가 몇 초 안에 정상적으로 잡힘. 예방책: SITL을 불필요하게 반복 `kill -9`하지 말 것(특히 디버깅 중 "혹시 중복 프로세스인가" 하고 습관적으로 죽이는 행동) — 매번 강제종료할 때마다 이 상태로 빠질 위험이 조금씩 누적되는 것으로 보임 |
