...
| 위 `parameters.bson` 삭제로도(옛 파일 그대로 둔 채 재시도만 반복한 경우) GPS/홈 위치 확인이 영영 안 넘어감(raw GPS는 `gps_info`/`raw_gps`로 보면 위성 10개·FIX_3D·hdop<1로 정상, ESTIMATOR_STATUS의 `flags`를 디코드해보면 `ESTIMATOR_CONST_POS_MODE` 비트가 켜져 있고 `ESTIMATOR_POS_HORIZ_ABS`가 꺼진 채 고정 — EKF2가 GPS를 아예 안 믿는 폴백 모드에 영구히 갇힌 상태. 180초 넘게 기다려도 그대로) | **2026-08-13~15에 여러 번 재현·재해결됨. 진짜 원인은 결국 `parameters.bson` 오염이었다** — 2026-08-14엔 "완전 클린 재빌드(`rm -rf build/px4_sitl_default` 후 재빌드)"로 해결된 것처럼 보였지만, 이는 클린 재빌드가 `parameters.bson`을 통째로 지우고 새로 만드는 부수효과 때문이었을 뿐 빌드 자체의 문제가 아니었던 것으로 최종 확인(2026-08-15, 동일 증상이 재발했을 때 **재빌드 없이 `parameters.bson`/`parameters_backup.bson`만 새 이름으로 백업**했더니 즉시 해결됨 — PX4 소스 코드를 원본으로 되돌려도 증상이 그대로 재현되는 대조실험으로 코드 변경이 원인이 아님도 확인함). 즉 클린 재빌드는 10분 넘게 걸리는 과잉 대응이고, **`mv rootfs/parameters.bson rootfs/parameters.bson.bak && mv rootfs/parameters_backup.bson rootfs/parameters_backup.bson.bak` 후 SITL 재시작이 훨씬 빠르고 정확한 해결책**. 예방책: SITL을 불필요하게 반복 `kill -9`하지 말 것 — 매번 강제종료할 때마다 이 파라미터 파일이 깨질 위험이 조금씩 누적되는 것으로 보임 |
