#!/bin/bash
# wind_random_dither_sweep.py(회전 토크 더더링 버전)를 여러 세션으로 나눠서 반복
# 실행하며, 세션 사이마다 SITL을 완전히 재시작하는 오케스트레이션 스크립트.
# run_yaw_collection_sessions.sh(무보정 버전)의 복사본 - wind_random_sweep.py 대신
# wind_random_dither_sweep.py를 부르는 것만 다름 (EXPERIMENTS.md 12-17절 참고).
#
...
#
# 사용법:
#   ./run_yaw_collection_dither_sessions.sh --sessions 6 --n-yaw 24 --n-per-yaw 10 --trial-duration 8
...
#   --trial-duration은 조건 하나당 관측 시간(초, wind_random_dither_sweep.py의 TRIAL_DURATION_S).
#   예) 2도 간격 + 약 13시간을 원하면:
#   ./run_yaw_collection_dither_sessions.sh --sessions 15 --n-yaw 12 --n-per-yaw 15 --trial-duration 15
#
...
# 중복 실행 방지 (2026-08-14 추가, run_yaw_collection_sessions.sh와 동일): 원인 불명이지만
# 실제로 같은 스크립트가 여러 개 동시에 떠서 SITL을 서로 뺏고 죽이며 GPS/홈 위치 확인이
# 영영 안 되는 문제를 반복 겪음. flock은 프로세스가 어떻게 죽든(kill -9 포함) OS가
# 자동으로 풀어주므로 /tmp/px4_lock-0류 파일 기반 락보다 안전함 - 두 번째 인스턴스는
# 바로 종료됨. 락 파일은 run_yaw_collection_sessions.sh(무보정 버전)와 일부러 공유함 -
# 어느 쪽이든 SITL을 독점해야 하는 건 똑같아서, 서로 다른 버전끼리 동시 실행되는 것도
# 막아야 함.
LOCKFILE="/tmp/run_yaw_collection_sessions.lock"
exec 200>"$LOCKFILE"
if ! flock -n 200; then
    echo "[중복 실행 감지] 이미 다른 run_yaw_collection_(dither_)sessions.sh 인스턴스가 실행 중입니다."
...
restart_sitl() {
    ...
    pkill -TERM -f "wind_random_dither_sweep.py" 2>/dev/null

    local term_waited=0
    while pgrep -f "px4_sitl_default/bin/px4|gz sim|mavsdk_server|wind_random_dither_sweep.py" > /dev/null \
          && [[ $term_waited -lt 8 ]]; do
        sleep 1
    ...
    pkill -9 -f "wind_random_dither_sweep.py" 2>/dev/null
...
for i in $(seq "$START_SESSION" "$SESSIONS"); do
    ...
    for attempt in $(seq 1 "$MAX_RETRIES"); do
        ...
        if (cd "$PROJECT_DIR/sim_scripts/data_collection" && \
            ...
            "$PX4SIM_PYTHON" -u wind_random_dither_sweep.py \
            ...
            session_ok=1
            if [[ "$PLOT_ENABLED" -eq 1 ]]; then
                latest_csv=$(ls -t "$PROJECT_DIR"/logs/wind_random_dither_*.csv 2>/dev/null | head -1)
...
echo "# 결과 CSV: ${PROJECT_DIR}/logs/wind_random_dither_*.csv"
echo "############################################################"
